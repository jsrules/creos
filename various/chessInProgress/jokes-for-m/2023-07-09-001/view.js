...
function putPiece(id, imgSrc) {
    ...
    $(id).innerHTML = imgSrc != EMPTY_STR ? '<img src="img/' + imgSrc + '.png">' : EMPTY_STR;
}
...
function fillStartPositionOBSOLETE() {
...
function redrawViewFromModel() {

    for (let index1 = 0; index1 < BOARD_SIZE; index1++) {
        let visualization = '';
        for (let index = 0; index < BOARD_SIZE; index++) {
            let piece = chessBoardArray[index1][index];
            let letter = getLetterOnTheRealBoardFromLetterNumber(index);
            let number = getNumberOnTheRealBoardFromNumber(index1);
            let id = letter + number
            if (piece == EMPTY) {
                putPiece(id, EMPTY_STR);
                continue;
            }
            let currColor = piece.color
            let currColorForImgFileName = currColor == WHITE_COLOR ? 'w' : 'b';
            visualization += piece.toString();
            putPiece(id, currColorForImgFileName + piece.toString());
        }
        l(visualization)
