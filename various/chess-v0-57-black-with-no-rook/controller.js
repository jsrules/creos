...
function fieldClickHandler(td) {
    ...
    if (lastClickedTD == td || (lastClickedTD != null && getColorOfPieceOnTd(td) == getColorOfPieceOnTd(lastClickedTD))) {
    ...
    if (lastClickedTD != null) {
        ...
        if (makeMoveIfPossible(firstClickTD, secondClickTD)) {
            ...
            //stupid ad hoc solution.. just hide Undo button in caSE of enPassant:
            if (!prevMoveWasEnPassantCapturing) {
                $('undoLastMoveLinkId').style.display = 'inline'
            } else {
                $('undoLastMoveLinkId').style.display = 'none'
                prevMoveWasEnPassantCapturing = false
            }
            ...
            if (!thereArePossibleMoves()) {
                isGameOver = true;
                $('undoLastMoveLinkId').style.display = 'none'
                $('whoMovesDivId').style.display = 'none'

                if (thereAintCheck()) {
                    myAlert('Draw. Stalemate')
                } else {
                    myAlert('Checkmate')
                }
