    ...
    for (let index1 = 0; index1 < FIELD_HEIGHT; index1++) {
        ...
        for (let index = 0; index < FIELD_WIDTH; index++) {
            ...
            if (isStart) {
                ...
                imgToDraw = modelArr[index][index1] == 0 ? imgToUse : (createId(index, index1) == bangMineId ? MINE_SQWARE_IMG : MINE_SQWARE_IMG_GREEN);
                if (isGameOver) {
                    gameOverMsg != FAIL_GAMEOVER_MESSAGE && (gameOverMsg = 'Success!!! Congrats, you won.')
                    ...
                    if (getFilenameFromURL(imgOnTheExistingBoard.src) == getFilenameFromURL(MARKED_SQWARE_IMG) && !isThereAMineAt(index, index1)) {
                        ...
                        gameOverMsg = FAIL_GAMEOVER_MESSAGE
                    }
                }
            }
