...
function circleStandardFilled(centerX, centerY, radius, colorBorder, colorFill) {
    ctx.beginPath();
    ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI, false);
    ctx.fillStyle =  colorFill; //"hsl(0deg 75% 25%)";  // randColor();//  colorFill;
    ctx.fill();
    ctx.lineWidth = 5;
    ctx.strokeStyle =  colorBorder //"hsl(360deg 75% 25%)";  //randColor();// colorBorder ;
    ctx.stroke();
    // ctx.lineWidth = 1;
    // ctx.moveTo(x1, y1);    // Передвигает перо в точку (30, 50)
    // ctx.lineTo(x2, y2);  // Рисует линию до точки (150, 100)
...
function circleStandardNOTFilled(centerX, centerY, radius, colorBorder, colorFill) {
    ctx.beginPath();
    ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI, false);
    // ctx.fillStyle =  colorFill; //"hsl(0deg 75% 25%)";  // randColor();//  colorFill;
    // ctx.fill();
